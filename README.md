...
<br> - Sau đó sẽ tạo ra một đối tượng là serializerReader thuộc kiểu JsonSerializerInternalReader. Chương trình tạo ra tiếp một đối tượng nữa là value để lưu giá trị của serializerReader sau khi được Deserialize.
<br> - Chạy qua một vài bước nữa (maybe để kiểm tra thôi :v ), thì chương trình sẽ gọi đến phương thức setter của thuộc tính MethodName của class ObjectDataProvider thuộc thư viện System.Windows.Data, Presentation Framework.dll (hình dưới)
...
<br> - Ta sẽ step over liên tục (chỗ này chưa hiểu là nó check cái gì -.-) thì chúng ta sẽ nhảy đến thứ mà ta cần tiếp theo, đó là thuộc tính ObjectInstance (hình dưới)
...
<br> Cứu :) đoạn này chưa hiểu, sau đó chương trình sẽ gọi Refresh()->BeginQuery()->QueryWorker()
<br> - Vì obj1 là Process (không null) nên nó sẽ gọi đến phương thức InvokeMethodOnInstance(). Thật tuyệt vời, _methodParameters.Count có giá trị là 2, vậy thì nhiều khả năng 2 giá trị đó chính là `cmd` và `calc`
mà trong payload có.
...
<br> - Và thật tuyệt vời, nó gọi đến phương thức Process.Start() trong lớp Process của thư viện System.Diagnostic. Ta thấy nó nhận 2 tham số của MethodParameter như hình dưới, với `fileName` là `cmd` và `arrguments` là `/c calc`. Vậy câu lệnh đầy đủ mà chương trình đã gọi là `System.Diagnostic.Process.Start("cmd", "calc")`
